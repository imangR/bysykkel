...
* `fread_trips_data()` fast reads bike trip data to R as a data frame by 
...
then the function returns a list that contains a dataframe, and a number that represents the
...
* There is an issue with `fread_trips_data()` in the current CRAN release of
bysykkel (i.e. version 0.1.1.0), which renders it unable to read bike trips 
data directly to R. If you want to use `fread_trips_data()`, then I recommend
that you download the development version of bysykkel from GitHub, where the 
issue is resolved.
